...
### Tree-shaking
The circuit needs to have a directed connection from the input to the output node. This is necessarily not true for all nodes after the pruning step, as some nodes might have lost all their parent connections during the pruning process. Tree-shaking removes these "dead nodes" from the network.
...
## Conclusion
ACDC is an exciting first step towards automating the circuit discovery process for neural networks. Figuring out ways to automate this process is crucial for making the mechanistic interpretability work scale to larger models and more complex tasks. 

### Limitations
Although this method is the first towards automatic circuit discovery process, it is not without its limitations. 
...
Secondly, the algorithm is computationally expensive to run, since it requires multiple forward passes through the network for each edge pruning step. This makes it difficult to apply to larger networks than the ones used in the paper.

### Upcoming frontiers for automated interpretability
I believe that there is a lot that can be done to extend the work presented in this paper. For example, different pruning strategies could be explored that would converge closer to the global optimal circuit. Additionally, this version of the algorithm is quite computationally expensive, so finding ways to make it more efficient would make it much more applicable to larger networks, both in terms of costs and time.
